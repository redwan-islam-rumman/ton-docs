...
```
2. No `impure`, it's by default, the compiler won't drop user function calls
...
7. A function can be called even if declared below; forward declarations are not needed; the compiler at first does parsing, and then it does symbol resolving; there is now an AST representation of source code
