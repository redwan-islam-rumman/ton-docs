...
const JETTON_WALLET_CODE = Cell.fromBoc(Buffer.from('b5ee9c72010101010023000842020f1ad3d8a46bd283321dde639195fb72602e9b31b1727fecc25e2edc10966df4', 'hex'))[0];
const JETTON_MASTER_ADDRESS = Address.parse('EQAJ8uWd7EBqsmpSWaRdf_I-8R8-XHwh3gsNKhy-UrdrPcUo');
const USER_ADDRESS = Address.parse('UQDKHZ7e70CzqdvZCC83Z4WVR8POC_ZB0J1Y4zo88G-zCSRH'); // Binance hot wallet
const hamster_merkle = 29945721131613672054990630604903528297803118035866103598622265652475246912273n;
const hamster_salt = 14n;

async function calcStateInit(
  owner: Address,
  master: Address,
  code: Cell,
  status?: number,
...
async function main() {
  const jettonWalletStateInit = await calcStateInit(USER_ADDRESS, JETTON_MASTER_ADDRESS, JETTON_WALLET_CODE, 0, hamster_merkle, hamster_salt);
